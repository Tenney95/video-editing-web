...
import { imageDecoder } from '@/utils/videoediting/webcodecs';
 ...
 */
export class ImageTrack implements VideoEditing.BaseTrackItem {
  id: string;
  type: VideoEditing.TrackType = 'image';
  source: VideoEditing.ImageSource;
  name: string;
  format: string;
  frameCount: number;
  start: number;
  ...
  constructor(source: VideoEditing.ImageSource, curFrame: number) {
    // 设置ID
    this.id = uniqueId();
    // 设置图片信息
    this.source = source;
  ...
  draw(ctx: CanvasRenderingContext2D, { width, height }: { width: number, height: number }, frameIndex: number) {
    ...
    return imageDecoder.getFrame(this.source.id, frame).then(vf => {
      if (vf) {
