...
import { checkTrackListOverlap } from '@/utils/videoediting/storeUtil';


export const useTrackState = defineStore('trackState', () => {
  const dragData = reactive({ // 拖拽数据
    dataInfo: {} as VideoEditing.TrackItem,
    dragType: '',
    dragPoint: {
      x: 0,
      y: 0
  ...
  const trackList = reactive<VideoEditing.TrackListEntry[]>([]);
   ...
   * 没有轨道时，新增轨道插入
   */
  function addTrack(newItem: VideoEditing.TrackItem) {
    const lines: any = trackList.filter(line => line.type === newItem.type);
