<template>
  <div class="trackList flex flex-1 flex-row w-full overflow-x-hidden overflow-y-auto relative">
    ...
    <div class="flex-1 overflow-x-ayto overflow-y-auto flex-col shrink-0 grow relative" ref="trackList"
      @scroll="handleScroll" @wheel="handleWheel" @click="setSelectTract($event, -1, -1)">
      <TimeLine :start="startX" :scale="trackScale" :step="defaultFps" @selectFrame="handlerSelectFrame" />
      <div id="orbit" class="absolute top-5 flex shrink-0 grow min-w-full"
        :style="{ 'min-height': 'calc(100% - 20px)' }" ref="trackListContainer" @mousedown="onMouseDown"
        @mousemove="onMouseMove" @mouseup="onMouseUp" @mouseleave="onMouseUp">
        <template v-if="showTrackList.length === 0">
          <div
            class="flex justify-center items-center h-24 m-auto w-2/3 dark:bg-gray-500 bg-gray-200  rounded-md text-sm border-dashed border-2 dark:border-gray-500 border-gray-200 hover:border-#683CFD">
            <icon-material-symbols:add-column-right-rounded class="text-8 mr-4" />
            将素材拖拽到这里，开始编辑你的大作吧~
          </div>
        </template>
        <div v-else class="z-10 pt-5 pb-5 min-w-full flex shrink-0 grow flex-col justify-center min-h-full"
          ...
          <template v-for="(lineData, lineIndex) of showTrackList"
            ...
            <div :style="{ height: TrackHeightMap.get(lineData.type) }">
              <TrackLine :style="{
        ...
        <template v-if="showTrackList.length !== 0">
          <div v-for="(line, i) in store.dragData.fixLines.reduce((r, item) => r.concat(item), [])" :key="i"
...
import TimeLine from '@/components/video-editing/item/trackItem/TimeLine.vue';
import TrackLine from '@/components/video-editing/item/trackItem/TrackLine.vue';
import TrackListIcon from '@/components/video-editing/item/trackItem/TrackListIcon.vue';
import TrackPlayPoint from '@/components/video-editing/item/trackItem/TrackPlayPoint.vue';
...
const offsetLine: VideoEditing.ItemLocation = {
...
let curCoord: VideoEditing.ItemLocation = { left: 0, right: 0, start: 0, end: 0 };
let otherCoords: VideoEditing.ItemLocation[] = [];
let dragElement: HTMLElement | null = null;

const showTrackList = computed<VideoEditing.ShowTrackList>(() => {
  return store.trackList.map((line, lineIndex) => {
    line.main && (mainIndex.value = lineIndex);
    const newList = line.list.map(item => {
      const { duration: time } = item.source as VideoEditing.VideoSource;
      return {
        ...item,
...
function isOverlap(dragItem: any, line: any, { start, end }: { start: number, end: number }) {
...
function getInsertLineInfo(): { isNewLine: boolean, insertIndex: number, elem?: HTMLElement } {
  if (!dragElement) return { isNewLine: false, insertIndex: -1 };
  const center = dragElement.offsetTop + dragElement.offsetHeight / 2 + (dragElement.offsetParent as HTMLElement)?.offsetTop;
...
let fixPosition: VideoEditing.ItemLocation = { left: 0, right: 0 };

// 获取插入信息
// insertIndex插入的位置，isNewLine是否插入新行, 插入的位置left、right值，插入行的位置，itemIndex
function getInsertInfo(): InsertInfo {
  if (!dragElement) return { insertIndex: -1, itemIndex: 0, left: 0, right: 0, isNewLine: false };
  ...
  if (overlap) {
    ...
    if (center < dragElement.offsetTop + (dragElement.offsetParent as HTMLElement)?.offsetTop + dragElement.offsetHeight / 2) {
      insertIndex -= 1;
    }
  }
