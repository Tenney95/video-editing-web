<template>
  <div ref="moveContainer" class="absolute left-0 right-0 top-0 bottom-0 m-auto">
    <div v-for="(item, index) in targetList" :key="item.id" :data-eleId="item.id" :data-lineIndex="item.lineIndex"
      :data-itemIndex="item.itemIndex" :style="{
        zIndex: index,
        top: `${item.top}px`,
        left: `${item.left}px`,
        width: `${item.w}px`,
      ...
      }" class="move-target absolute" @click.stop="selectItem(item.id)" @mousedown="mousedown($event, item.id)" />
    <Moveable ref="moveable" v-bind="draggableOptions" @drag="onDrag" @scale="onScale" />
  </div>
</template>
...
const targetList = computed(() => {
  ...
  trackStore.trackList.forEach(({ list }, lineIndex) => {
    ...
    const trackItem: VideoEditing.TrackItem = list[index];
    if (trackItem) {
      layerArr.unshift({
        lineIndex,
        itemIndex: index,
        ...
        x: trackItem.centerX || 0,
        y: trackItem.centerY || 0,
        w: trackItem.width || 0,
        h: trackItem.height || 0,
