<template>
  <div class="absolute left-0 right-0 top-0 bottom-0 border z-20"
    :class="{ 'dark:border-gray-100 border-gray-600': isActive }" ref="el" v-show="isActive">
    <div
      ...
      ref="handlerLeft" @mousedown="mouseDownHandler($event, 'left')">
      <span>|</span>
    </div>
    <div
      ...
      ref="handlerRight" @mousedown="mouseDownHandler($event, 'right')">
      <span>|</span>
    </div>
  </div>
</template>
...
import { getGridPixel } from '@/utils/videoediting/canvasUtil';
...
// let fixPosition: any = { left: 0, right: 0, start: 0, end: 0 };
...
function initLimits(lineData: VideoEditing.BaseTrackItem[], trackItem: VideoEditing.BaseTrackItem) {
  ...
  if (isVA) { // 音视频结尾受资源大小限制
    const rightMaxWidth = (trackItem.frameCount - (limitData.offsetL ?? 0)); // 右侧最大宽度
    const leftMaxWidth = (trackItem.frameCount - (limitData.offsetR ?? 0));// 左侧最大宽度
